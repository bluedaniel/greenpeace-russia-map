...
function addMarker(marker, content) {
    google.maps.event.addListener(marker, 'click', function () {
        infoWindow.setContent('<div id="contentWindow">' + content.join('') + '</div>');
...
function initializeMap(mapData) {
    ...
    for (var i = 0; i < mapData.length; i++) {
        ...
        for (var j = 0; j < locations.length; j++) {
            ...
            var markerIcon = new google.maps.MarkerImage(markerIconUrl, null, null, null, new google.maps.Size(28, 28));
            ...
            var keyPoints = [];
            keyPoints.push('<b>Partners</b><span class="partners">' + item.partners + '</span>');
            keyPoints.push('<b>Stage</b><span class="stage">' + item.stage + '</span>');
            content.push('<div class="key-points">' + keyPoints.join('') + '</div>');

            content.push('<span class="info">' + item.size + '</span>');
            content.push('<span class="more-info"><a href="' + item.link + '" target="_blank">More Info</a></span>');
